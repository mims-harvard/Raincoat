...
class cross_domain_trainer_ours(object):
    """
   This class contain the main training functions for our AdAtime
        ...
        for i in scenarios:
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                for epoch in range(1, self.hparams["num_epochs"] + 1):
                    ...
                    if f1>=self.best_acc:
                        self.best_acc = f1
                        torch.save(self.algorithm.encoder.state_dict(), self.fpath)
    ...
    def save_best(self):
        ...
        with torch.no_grad():
            ...
            # for data, labels in self.src_train_dl:
                ...
                self.src_true_labels = np.append(self.src_true_labels, labels.data.cpu().numpy())
            accuracy = accuracy_score(self.src_true_labels, self.src_pred_labels)
            self.trg_acc_list.append(accuracy)
            f1 = f1_score(self.src_pred_labels, self.src_true_labels, pos_label=None, average="macro")
        return f1
    ...
    def save_result(self, df, name):
        ...
        result = pd.concat(objs=(iDF.set_index('scenario') for iDF in (mean_acc, mean_f1,std_acc,std_f1)),
